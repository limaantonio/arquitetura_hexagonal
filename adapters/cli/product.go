...
func Run(service application.ProductServiceInterface, action string, productId string, productName string, price float64) (string, error) {
	...
	case "create":
		...
		result = fmt.Sprintf("Product ID %s with the name %s and price %.2f and status %s was created", product.GetID(), product.GetName(), product.GetPrice(), product.GetStatus())
